...
- XML sitemap: run `$CLAUDE_PROJECT_DIR/.claude/skills/seo-agent/bin/claude-seo run sitemap_discovery.py <url> --json`; require a
...
/.claude/skills/seo-agent/bin/claude-seo run agent_ux_check.py https://example.com --json
```
...
without scoring, use `$CLAUDE_PROJECT_DIR/.claude/skills/seo-agent/bin/claude-seo run render_page.py <url> --a11y-tree --json`.
