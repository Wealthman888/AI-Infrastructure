...
# SEO Agent: Universal SEO Analysis Skill

**Invocation:** `/seo $1 $2` where `$1` is the command and `$2` is the URL or argument.

**Runtime:** Run bundled Python tools through `$CLAUDE_PROJECT_DIR/.claude/skills/seo-agent/bin/claude-seo run <script.py>`. Plugin
...
to repair dependencies. Execute `$CLAUDE_PROJECT_DIR/.claude/skills/seo-agent/bin/claude-seo setup`, report core and Chromium
status separately, and do not fall back to global or user package installation.
For diagnosis, execute `$CLAUDE_PROJECT_DIR/.claude/skills/seo-agent/bin/claude-seo doctor --json`; its output intentionally omits
absolute paths and environment values. If any `$CLAUDE_PROJECT_DIR/.claude/skills/seo-agent/bin/claude-seo run` command reports
...
3. If Google API credentials detected (`$CLAUDE_PROJECT_DIR/.claude/skills/seo-agent/bin/claude-seo run google_auth.py --check`), also spawn seo-google agent
...
6. If backlink APIs detected (`$CLAUDE_PROJECT_DIR/.claude/skills/seo-agent/bin/claude-seo run backlinks_auth.py --check`), also spawn seo-backlinks agent
...
10. If drift baseline exists for this URL (`$CLAUDE_PROJECT_DIR/.claude/skills/seo-agent/bin/claude-seo run drift_history.py <url>`), also spawn seo-drift agent
