...
- Fetch the target URL using `$CLAUDE_PROJECT_DIR/.claude/skills/seo-agent/bin/claude-seo run render_page.py "<url>" --mode auto --json` (SPA-aware SSRF-protected renderer)
- Parse with `$CLAUDE_PROJECT_DIR/.claude/skills/seo-agent/bin/claude-seo run parse_html.py --url "<url>"` to extract SEO elements
