...
skills: .claude/skills/cfo-advisor
domain: c-level
model: opus
...
**Skill Location:** `.claude/skills/cfo-advisor/`

### Python Tools

1. **Burn Rate Calculator**
   - Path: `.claude/skills/cfo-advisor/scripts/burn_rate_calculator.py`
   - Usage: `python .claude/skills/cfo-advisor/scripts/burn_rate_calculator.py`
   - Outputs base/bull/bear runway scenarios, months-of-cash, default-alive vs default-dead status

2. **Unit Economics Analyzer**
   - Path: `.claude/skills/cfo-advisor/scripts/unit_economics_analyzer.py`
   - Usage: `python .claude/skills/cfo-advisor/scripts/unit_economics_analyzer.py`
   - Per-cohort LTV, per-channel CAC, payback months, gross margin breakdown

3. **Fundraising Model**
   - Path: `.claude/skills/cfo-advisor/scripts/fundraising_model.py`
   - Usage: `python .claude/skills/cfo-advisor/scripts/fundraising_model.py`
   - Dilution modeling, cap table projections, round sensitivity, valuation negotiation ranges

### Knowledge Bases

- `.claude/skills/cfo-advisor/references/financial_planning.md` — modeling, FP&A cadence, scenario design
- `.claude/skills/cfo-advisor/references/fundraising_playbook.md` — round preparation, term sheet decoding, investor outreach
- `.claude/skills/cfo-advisor/references/cash_management.md` — treasury, working capital, AR/AP discipline
...
```bash
python .claude/skills/cfo-advisor/scripts/burn_rate_calculator.py > runway.txt
```
...
python .claude/skills/cfo-advisor/scripts/burn_rate_calculator.py > /tmp/burn.txt
python .claude/skills/cfo-advisor/scripts/unit_economics_analyzer.py > /tmp/ue.txt
python .claude/skills/cfo-advisor/scripts/fundraising_model.py > /tmp/fund.txt
...
- Skill: [.claude/skills/cfo-advisor/SKILL.md](.claude/skills/cfo-advisor/SKILL.md)
